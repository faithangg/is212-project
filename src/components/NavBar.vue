<template>
  <v-layout>
    ...
    >
      ...
      <v-app-bar-title style="text-align: left;"> <img v-bind:src="require('../assets/sbrp.png')" style="width: 200px; margin-top: 20px;"/></v-app-bar-title>
      
      <v-spacer></v-spacer>

      <v-btn v-if="isLoggedIn" to="/viewRolesPage" id="role_listings">View Role Listings</v-btn>

      <v-btn v-if="userIsHr" to="/manageRolesPage" id="managed">Manage Role Listings</v-btn>
      
      <v-btn v-if="isLoggedIn" to="/profile" id="profile">Profile</v-btn>
